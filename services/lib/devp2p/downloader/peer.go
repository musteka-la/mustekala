...
func (p *peerConnection) setIdle(started time.Time, delivered int, throughput *float64, idle *int32) {
	...
	*throughput = (1-measurementImpact)*(*throughput) + measurementImpact*measured
	...
	log.Debugf("Peer throughput measurements updated hps %v bps %v rps %v sps %v miss %v rtt %v",
		p.headerThroughput, p.blockThroughput,
		p.receiptThroughput, p.stateThroughput,
		len(p.lacking), p.rtt)
}
